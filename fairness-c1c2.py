...
file1 = open("output/output_c1c2/output_c1c2.txt","w")
...
(X_train, X_test, y_train, y_test, _, Z_test_race, _, Z_test_sex, Z_train_c1, Z_test_c1, Z_train_c2, Z_test_c2) = train_test_split(X, y, Z_race, Z_sex, Z_c1, Z_c2,
                                                                                                                          test_size=0.5, stratify=y, random_state=7)
Z_sets = [Z_test_c1, Z_test_c2, Z_test_race, Z_test_sex]
Z_sets_names = ['Z_test_c1', 'Z_test_c2', 'Z_test_race', 'Z_test_sex']
...
file1.writelines([f"features X: {X.shape[0]} samples, {X.shape[1]} attributes \n",
                  ...
                  f"sensitives Z_train_c2: {Z_train_c2.shape[0]} samples, {Z_train_c2.shape[1]} attributes \n",
...
train_data = PandasDataSet(X_train, y_train, Z_train_c1, Z_train_c2)
...
def plot_distributions(y_true, Z_true, y_pred, Z_test_name, image_name, Z_pred = None, epoch=None):
    ...
    _subplot(subplot_df, protected, Z_test_name, ax = axes)
    _performance_text(fig, y_true, Z_true, y_pred, image_name, Z_test_name, protected, Z_pred, epoch)
    fig.tight_layout()
    return fig
...
def _performance_text(fig, y_test, Z_test, y_pred, image_name, Z_test_name, protected, Z_pred=None, epoch=None):

    file1.writelines(f"{image_name}_{Z_test_name} \n")
    file1.writelines("\n")
...
def test(image_name):
    ...
    for Z_test, Z_test_name in zip(Z_sets, Z_sets_names):
        ...
        fig.savefig('output/output_c1c2/{}_{}.png'.format(image_name, Z_test_name))
...
def pretrain_adversary2(adv, clf, data_loader, optimizer, criterion):
    for x, _, _, z2 in data_loader:
        p_y = clf(x).detach()
        adv.zero_grad()
        p_z = adv(p_y)
        loss = criterion(p_z, z2).mean() 
        loss.backward()
        optimizer.step()
    return adv

adv1 = Adversary(n_features = 1, n_sensitive = Z_train_c1.shape[1])
adv2 = Adversary(n_features = 1, n_sensitive = Z_train_c2.shape[1])
adv_criterion = nn.BCELoss(reduce=False)
adv1_optimizer = optim.Adam(adv1.parameters())
adv2_optimizer = optim.Adam(adv2.parameters())

N_ADV_EPOCHS = 5

for epoch in range(N_ADV_EPOCHS):
    adv1 = pretrain_adversary1(adv1, clf, train_loader, adv1_optimizer, adv_criterion)
    adv2 = pretrain_adversary2(adv2, clf, train_loader, adv2_optimizer, adv_criterion)
...
def train(clf, adv1, adv2, data_loader, clf_criterion, adv_criterion,
          clf_optimizer, adv1_optimizer, adv2_optimizer):
    
    # Train both adversaries
    for x, y, z1, z2 in data_loader:
        p_y = clf(x)
        adv1.zero_grad()
        adv2.zero_grad()
        p_z1 = adv1(p_y)
        p_z2 = adv2(p_y)  
        loss_adv = adv_criterion(p_z1, z1).mean() + adv_criterion(p_z2, z2).mean()
        loss_adv.backward()
        adv1_optimizer.step()
        adv2_optimizer.step()
    ...
    return clf, adv1, adv2

N_EPOCH_COMBINED = 165

for epoch in range(1, N_EPOCH_COMBINED):
    
    clf, adv1, adv2 = train(clf, adv1, adv2, train_loader, clf_criterion, adv_criterion,
                     clf_optimizer, adv1_optimizer, adv2_optimizer)

    with torch.no_grad():
        clf_pred = clf(test_data.tensors[0])
        adv_pred1 = adv1(clf_pred)
        adv_pred2 = adv2(clf_pred)
