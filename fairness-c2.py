...
file1 = open("output/output_c2/output_c2.txt","w")
...
(X_train, X_test, y_train, y_test, _, Z_test_race, _, Z_test_sex, _, Z_test_c1, Z_train_c2, Z_test_c2) = train_test_split(X, y, Z_race, Z_sex, Z_c1, Z_c2, test_size=0.5, stratify=y, random_state=7)

Z_train_c1 = Z_train_c2                                                                                                                          
...
def plot_distributions(y_true, Z_true, y_pred, name, image_name, Z_pred = None, epoch=None):
    ...
    _subplot(subplot_df, protected, name, ax = axes)
...
def test(image_name):
    ...
    for Z_test, Z_test_name in zip(Z_sets, Z_sets_names):
        ...
        fig.savefig('output/output_c2/{}_{}.png'.format(image_name, Z_test_name))
...
for epoch in range(1, N_EPOCH_COMBINED):
    ...
    clf, adv1 = train(clf, adv1, train_loader, clf_criterion, adv_criterion,
                     clf_optimizer, adv1_optimizer)
