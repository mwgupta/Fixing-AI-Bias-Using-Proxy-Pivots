...
file1 = open("output/output_v3/output_race/output_race.txt","w") 
...
(X_train, X_test, y_train, y_test, Z_train_race, Z_test_race, _, Z_test_sex, _, Z_test_c1, _, Z_test_c2) = train_test_split(X, y, Z_race, Z_sex, Z_c1, Z_c2,
...
file1.writelines([f"features X: {X.shape[0]} samples, {X.shape[1]} attributes \n",
                  ...
                  f"sensitives Z_train_race: {Z_train_race.shape[0]} samples, {Z_train_race.shape[1]} attributes \n",
...
train_data = PandasDataSet(X_train, y_train, Z_train_race)
...
def test(image_name):
    ...
    for Z_test, Z_test_name in zip(Z_sets, Z_sets_names):
        ...
        fig.savefig('output/output_v3/output_race/{}_{}.png'.format(image_name, Z_test_name))
...
adv1 = Adversary(n_features = 1, n_sensitive = Z_train_race.shape[1])
adv_criterion = nn.BCELoss(reduce=False)
adv1_optimizer = optim.Adam(adv1.parameters(), lr = 0.02, weight_decay = 0.1)
...
plt.savefig('output/output_v3/output_race/entropy_fig.png')

plt.figure()
plt.plot(plt_clf_loss, 'bo', plt_adv_loss, 'r+')
plt.savefig('output/output_v3/output_race/loss_fig.png')
